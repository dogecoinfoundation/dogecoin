...
class GenerateBlockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        txid = node.sendtoaddress(address, 10)
        ...
        raw = node.createrawtransaction([{'txid':utxos[0]['txid'], 'vout':utxos[0]['vout']}],[{address:10}])
        ...
        raw1 = node.createrawtransaction([{'txid':txid, 'vout':0}],[{address:9.9}])
        signed_raw1 = node.signrawtransactionwithwallet(raw1)['hex']
        txid1 = node.sendrawtransaction(signed_raw1)
        raw2 = node.createrawtransaction([{'txid':txid1, 'vout':0}],[{address:9.0}])
